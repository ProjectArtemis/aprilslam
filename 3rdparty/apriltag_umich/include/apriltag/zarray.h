 ...
 * the specified size. It is the caller's responsibility to call
 * zarray_destroy()
 ...
 * created by zarray_create(). After calling, 'za' will no longer be valid for
 * storage.
 */
 ...
 * array, which may be different from its capacity. The index of the last
 * element
 ...
 * guarantee that the supplied number of elements (capacity) can be safely
 * stored.
 */
 ...
 * Automatically ensures that enough storage space is available for the new
 * element.
 */
 ...
 * index of 'idx' and copies its value into the variable pointed to by the
 * pointer
 * 'p'.
 */
 ...
 * 'p' should be a pointer to the pointer which will be set to the internal
 * address.
 */
...
/**
 * Sets the value of the current element at index 'idx' by copying its value
 * from
 ...
 *
 * The comparison function will be passed a pointer to two elements to be
 *compared
 ...
 * element is 'greater than' the second. The function should have the following
 *format:
 *
 * int comparison_function(const element_type *first, const element_type
 **second)
 *
...
/**
 * A comparison function for comparing strings which can be used by
 * zarray_sort()
