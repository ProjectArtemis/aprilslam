...
static void neighbor_search_task(void *_u)
{
  ...
  for (int i = task->i0; i < task->i1; i++)
  {
    ...
    while (1)
    {
      ...
      float neighbor_cost = K1 * fabs(dtheta - M_PI / 2) + K2 * sqrtf(dist2 / length2) + K3 * sqrtf(dist2) + K4;
  ...
  if (1)
  {
    ...
    for (int i0 = 0; i0 < zarray_size(detections); i0++)
    {
      ...
      for (int i1 = i0 + 1; i1 < zarray_size(detections); i1++)
      {
        ...
        /*
           printf(" %d %d (%d) %d %d\n", i0, i1, zarray_size(detections), g2d_polygon_overlaps_polygon(poly0, poly1), g2d_polygon_overlaps_polygon(poly1, poly0));
           */
