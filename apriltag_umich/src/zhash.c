...
zhash_t *zhash_copy(zhash_t *orig) {
  ...
  zhash_t *out =
      zhash_create(orig->keysz, orig->valuesz, orig->hash, orig->equals);
...
static inline int zhash_put_real(zhash_t *zh, struct bucket *buckets,
                                 int nbuckets, const void *key,
                                 const void *value, void *old_key,
                                 void *old_value) {
  assert(zh != NULL);
  assert(buckets != NULL);
  assert(key != NULL);
...
int zhash_put(zhash_t *zh, const void *key, const void *value, void *oldkey,
  ...
  int has_oldkey = zhash_put_real(zh, zh->buckets, zh->nbuckets, key, value,
                                  oldkey, oldvalue);
