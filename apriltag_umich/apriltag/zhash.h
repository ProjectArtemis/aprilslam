 ...
 *          for structs: zhash_create(sizeof(struct key_struct), sizeof(struct
 *value_struct)...)
 ...
 *to the start
 *     of 'key_size' and 'value_size' bytes, which you have already malloc'd or
 *stack allocated
 *   - This data structure can be used to store types of any size, from bytes &
 *doubles to
 *     user defined structs
 *     Note: if zhash stores pointers, user must be careful to manually manage
 *the lifetime
...
/**
 * Returns an array which contains copis of all of the hash table's values, in
 * no
 * particular order. It is the caller's responsibility to call zarray_destroy()
 * on the returned structure when it is no longer needed.
 */
