...
void cam_callback(const sensor_msgs::Image::ConstPtr &img) {
  ...
  if (img->encoding == sensor_msgs::image_encodings::MONO8) {
    //  use existing data
    image_gray = cv::Mat(cv::Size(img->width, img->height), CV_8UC1,
  ...
  for (vector<AprilTags::TagDetection>::iterator I = detections.begin();
       I != detections.end(); I++) {
    for (int j = 0; j < 4; j++) {
...
int main(int argc, char **argv)
{
  ...
  image_transport::Subscriber camSub =
      imgTransport.subscribe("/camera/image_raw", 1, cam_callback);
